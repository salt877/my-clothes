$(function() {
	...
	$(function() {
		$('#brand').on("keyup", function() {
			var hostUrl = 'http://localhost:8080/check_brand_api/brandcheck';// どのクラスのどのメソッドなのかを指定
			var inputBrand = $("#brand").val(); // idと結びついている
			...
			$.ajax({
				...
				data : {
					brand : inputBrand　　　　// 引数の中身を指定？（イメージはkey,value)keyは自由に設定
				},
				...
				$("#duplicateMessage").html(data.duplicateMessage);　// jqueryをつかって、htmlの<div
																	...
																	// aこの部分を変更していく（今回はlistに入れていくイメージ）
